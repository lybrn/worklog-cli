...
class Parse2 {
  ...
  public function decode($contents) {
    ...
    // use "Entities" to parse the file
  ...
  public function parse($contents) {
    ...
    foreach($lines as $i=>$line) {
      if ($depth = Parse2::get_depth($lines,$i,$line,$current_style,$current_depth)) {
        $depth_offset = Parse2::get_depth_offset($lines,$i,$line,$current_style,$current_offset);
        $style = Parse2::get_style($lines,$i,$line);
        $matched = Parse2::get_matched($lines,$i,$line);
        $current_style = $style;
        $current_depth = $depth;
  ...
  public function get_depth_offset($lines,$i,$line,$current_style,$current_offset) {
    ...
    // get indent depth
    $previndent = Parse2::get_line_indent_depth($prevline);
    $indent = Parse2::get_line_indent_depth($line);
    $nextindent = Parse2::get_line_indent_depth($nextline);
