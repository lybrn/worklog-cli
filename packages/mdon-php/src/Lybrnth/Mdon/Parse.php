...
class Parse {
  ...
  public function parse($contents) {
    ...
    foreach($lines as $i=>$line) {
      if ($depth = Parse::get_depth($lines,$i,$line,$current_style,$current_depth)) {
        $depth_offset = Parse::get_depth_offset($lines,$i,$line,$current_style,$current_offset);
        $style = Parse::get_style($lines,$i,$line);
        $matched = Parse::get_matched($lines,$i,$line);
        $current_style = $style;
        $current_depth = $depth;
  ...
  public function get_depth($lines,$i,$line,$current_style,$current_depth) {
    ...
    // [indent] item --- this line starts with "[indent]"
    if (preg_match('/^\s\s/i',rtrim($line))) {
      // if the curent style from previous line is [indent] match current depth
      if (in_array($current_style,array('[indent]'))) {
        // print "== ".$line."\n";
        return $current_depth;
      } 
      // if the current style from previous line is anything else - increment depth
      else {
        // print "++ ".$line."\n";
        return $current_depth + 1;
      }
  ...
  public function get_depth_offset($lines,$i,$line,$current_style,$current_offset) {
    ...
    // get indent depth
    $previndent = Parse::get_line_indent_depth($prevline);
    $indent = Parse::get_line_indent_depth($line);
    $nextindent = Parse::get_line_indent_depth($nextline);
